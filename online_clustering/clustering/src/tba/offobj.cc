...
OffObj::OffObj() : 
  bbmin(9999999,9999999,9999999), 	
  bbmax(-9999999,-9999999,-9999999), is_coff(false)
{
...
void OffObj::orient_facets() {
  ...
  for(i=0; i<topo_facets[0].size(); i++) {
    facet_queue.push_back(make_pair<int, int>((int)topo_facets[0][i], 0));
  ...
  while(facet_queue.size() != 0) {
	...
	facet_queue.push_back(make_pair<int, int>((int)neighbor, (int)facet_info.first));
    ...
    if (flip_facet) {
      ...
      //flip normal
      Eigen::Vector3d normal = facet_normals[facet_info.first];
...
void OffObj::calculate_facet_normals() {
  ...
  for(i=0; i<facets.size(); i++) {
    ...
    if (facet.size() > 2) {
      Eigen::Vector3d pt0 = vertices[facet[0]];
      Eigen::Vector3d pt1 = vertices[facet[1]];
      Eigen::Vector3d pt2 = vertices[facet[2]];
...
void OffObj::calculate_vert_normals() 
{
  ...
  for(uint i=0; i<vertices.size(); i++) {
    ...
    for(uint j=0; j<topo_verts[i].size(); j++) {
      normal += facet_normals[topo_verts[i][j]];
    }
    normal /= topo_verts[i].size();
    normal.normalize();
...
bool OffFileReader::get_next_off_object(OffObj& o) {
  ...
  while(!found_off && get_next_line(tokens)) {
	...
	o.vertices.push_back(Eigen::Vector3d(x, y, z));
	...
	Eigen::Vector3d	p = o.vertices[facet[0]];
	if (tokens.size() == 1+verts_in_facet) {
	  //some default color
	  /*	
		float delta[3] = {0};
		delta[0] = bbmax(0) - bbmin(0);
		delta[1] = bbmax(1) - bbmin(1);
		delta[2] = bbmax(2) - bbmin(2);
		r = p(0)/delta[0]/2.0 + 1.0;
		g = p(1)/delta[1]/2.0 + 1.0;
		b = p(2)/delta[2]/2.0 + 1.0;
		a = 1.0f;
	  */	
