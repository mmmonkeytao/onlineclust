...
#define READ_FILE(fvar,fname) \
   ...
   if (!fvar) throw GPLIB::GP_Exception("Could not open file % ",fname, __FILE__, __LINE__);
#define WRITE_FILE(fvar,fname) \
  std::ofstream fvar(fname);					\
  if (!fvar) throw GPLIB::GP_Exception("Could not open file %s",fname, __FILE__, __LINE__);
