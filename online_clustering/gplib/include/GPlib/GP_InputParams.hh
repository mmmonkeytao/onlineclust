...
namespace GPLIB {
  ...
  struct GP_InputParams
  {
    ...
    uint gtsrb_sep1, gtsrb_sep2; //!< the separator for each class in GTSRB data
    ...
    uint batch_size; //!< number of steps used for a batch update
    double max_entropy; //!< everything more than that gets re-asked
    uint nb_questions; //!< number of questions asked in active learning
    uint nb_iterations; //!< number of iterations for IVM training
    int label1, label2, label3; //!< labels wanted from the input data
    std::vector<std::pair<int, int> > label_map; 
    ...
    uint dim_red_pca; //!< dimensionality reduction, if 0 no reduction is done 
    ...
    std::vector<double> opt_params; // Optimiziation parameters (step size, tolerance, convergence threshold)
    GP_Optimizer::OptimizerType opt_type;
