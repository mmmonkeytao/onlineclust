...
namespace GPLIB {
  ...
  public:
    ...
    // returns true if an entry with the given output value exists
    bool Has(OutputType const &out) const
    {
    ...
    {
      return CalcInputMean(InputType());
    }

    /*
    double CalcInputMean(double unused) const
    {
      if(_xvec.size() == 0)
	return 0;
    ...
    std::vector<double> CalcInputMean(std::vector<double> unused) const
    {
      if(_xvec.size() == 0)
	return std::vector<double>();
      
      std::vector<double> mean = _xvec[0];
      for(uint i=1; i<_xvec.size(); ++i)
     ...
     * Returns the mean of all input values
     */
    InputType CalcCommonInputMean(GP_DataSet<InputType, OutputType> const &other) const
    {
      if(_xvec.size() == 0 && other.Size() == 0)
	return InputType();

      GP_Vector mean = _xvec[0];
      for(uint i=1; i<_xvec.size(); ++i)
	mean += GP_Vector(_xvec[i]);

      for(uint i=0; i<other._xvec.size(); ++i)
	mean += GP_Vector(other._xvec[i]);
      
      mean /= (_xvec.size() + other._xvec.size());
      return mean;
    }
  ...
  void WritePNM(std::string filename, uint wid, uint hght) const
    {
      uint dim = GetInputDim();
      if(dim == 5){ // two position and three color values
	
	WRITE_FILE(pfile, filename.c_str());
	pfile << "P3" << std::endl;
	pfile << wid << " " << hght << " 255" << std::endl;
	for(uint j=0; j<hght; ++j){
	  for(uint i=0; i<wid; ++i){
	    uint k = i * hght + j;
	    if(_yvec[k] == 1)
	      pfile << _xvec[k][4] << " " << _xvec[k][3] << " " << _xvec[k][2] << " ";
	    else{
	      pfile << "180 0 0 ";
	    }
	  }
  ...
  private:
    ...
    uint GetDimension(std::pair<T, W> const &vec) const
    {
      return GetDimension(vec.first);
    }
    
    void AddVec(double &v, uint i) const
    {
      v += _xvec[i];
    }

    void AddVec(GP_Vector &v, uint i) const
    {
      v += _xvec[i];
    }
    ...
    int Read(GP_Vector x, int y, std::string filename, int pos = 0) 
    {
      READ_FILE(gfile, filename.c_str());
      gfile.seekg(pos);
      uint size, dim;
	...
	for(uint j=0; j<discard[i]; ++j){
      ...
      std::cout << to_remove.size() << " samples" << std::endl;
