...
namespace GPLIB {
  ...
  void GP_InputParams::Read(std::string const &filename)
  {
	...
	ifile >> label3;
      ...
      else if(tag == "LABEL_MAP"){
	std::vector<double> lvec = ReadVector(ifile);
	for(uint i=0; i<lvec.size(); i += 2)
	  label_map.push_back(std::make_pair(lvec[i], lvec[i+1]));
      }
	...
	ifile >> shuffle; 
      ...
      else if(tag == "OPT_PARAMS"){
	opt_params = ReadVector(ifile);
      }
      else if(tag == "OPT_TYPE"){
	std::string optname;
	ifile >> optname;
	if(optname == "FR"){
	  opt_type = GP_Optimizer::FR;
	}
	else if(optname == "PR"){
	  opt_type = GP_Optimizer::PR;
	}
	else if(optname == "LBFGS"){
	  opt_type = GP_Optimizer::LBFGS;
	}
      }
      else if(tag == "VERBOSE"){
	ifile >> verbose; 
  ...
  void GP_InputParams::Write(std::string filename) const
  {
    ...
    ofile << "LABEL_MAP\t\t\t\t" ;
    for(uint i=0; i<label_map.size(); ++i)
      ofile << label_map[i].first << " " << label_map[i].second << " ";
    ofile  << std::endl;
